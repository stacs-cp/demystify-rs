#!/usr/bin/env bash
# Re-run futoshiki benchmarks with --only-assign for comparison.
set -eu

OUTDIR="${BENCH_OUTDIR:-$TMPDIR/bench-futoshiki-oa}"
...
for diff in 4x4-easy 5x5-easy 5x5-normal 5x5-hard 9x9-easy 9x9-normal 9x9-hard; do
    echo -n "  futoshiki-oa-${diff} "
    for nn in $INSTANCES; do
        param="${EPRIME}/futoshiki/puzzle-futoshiki-com/${diff}-${nn}.param"
        if [ ! -f "$param" ]; then
            echo -n "?"
            continue
        fi
        run_one "futoshiki-oa-$diff" "$nn" standard "$EPRIME/futoshiki.eprime" "$param"
        run_one "futoshiki-oa-$diff" "$nn" core "$EPRIME/futoshiki.eprime" "$param"
        run_one "futoshiki-oa-$diff" "$nn" "core+mus" "$EPRIME/futoshiki.eprime" "$param"
        echo -n "."
    done
    echo " done"
done
